...
def generate_dirichlet_instance(N,P,R,concentration_parameter,density_of_b1,b2maxfactor,b3max):
    np.random.seed(N+P+R) # fix the pseudorandom seed,if desired
...
def plot_dirichlet_distributions(α_prior, α_post, α_tgt):
    ...
    for alpha, label in zip(alphas, labels):
        ...
        plt.savefig(f"C:/Users/brian/Documents/PA/Plots/DM_IPA_{label[0:5]}.png", dpi=300, bbox_inches='tight')     
        plt.show()

def plot_poisoned_dirichlet_distributions(α_poisoned, title):
    def format_alpha(alpha):
        return r"$\alpha = [" + ", ".join(map(str, alpha)) + "]$"
    alphas = [α_poisoned]
    labels = [
    ...
    for alpha, label in zip(alphas, labels):
        ...
        cbar = fig.colorbar(contour, ax=ax, shrink=0.8)
        cbar.set_label("Density")
        plt.savefig(f"C:/Users/brian/Documents/PA/Plots/DM_IPA_{title}.png", dpi=300, bbox_inches='tight')     
        plt.show()
...
def solve_Q2L1(N,P,α_prior,Xprime,b1,b2,b3,α_tgt,precision,MP_time_limit,solver_name):
    start_time = time.time() # Start the clock
    grad_list = approximate_gradient(α_prior,Xp,α_tgt,precision)
    H_list = approximate_hessian(α_prior,Xp,α_tgt,precision)
    ...
    model.f=pyo.Param(initialize=kl_dirichlet_multinomial_given_X(α_prior,Xprime,α_tgt))
    # Decision Variables
    ...
    # Formulate model
    # Objective function 
    model.objfnvalue = pyo.Objective(expr = model.f+sum(sum(model.grad[n,p]*(model.X[n,p]-model.Xp[n,p]) for n in model.N) for p in model.P)+(1/2)*sum(sum(sum(sum(model.H[n,p,k,l]*(model.X[n,p]-model.Xp[n,p])*(model.X[k,l]-model.Xp[k,l]) for n in model.N) for p in model.P) for k in model.N) for l in model.P),sense = pyo.minimize)
...
def run_methods(α_prior,N,P,R,Xp,b1,b2,b3,α_tgt,precision,MP_time_limit,QA_max_time,QA_max_iterations,QA_termination_tolerance):
    ...
    if P==3:
        plot_dirichlet_distributions(α_prior,α_post,α_tgt)
    '''
    for solver_name in solvers: 
        print('******************************')
        print(f"Q2L1 Results with {solver_name}")
        solution_Q2L1=solve_Q2L1(N,P,α_prior,Xp,b1,b2,b3,α_tgt,precision,MP_time_limit,solver_name)
        X=Xp
        X=np.array(solution_Q2L1['X'])
        #print(f"X={X}")
        ...
        if P==3:
            ...
            plot_poisoned_dirichlet_distributions(α_poisoned, title)
    '''    
    for solver_name in solvers: 
        print('******************************')
...
(Xp,b1,b2,b3)=generate_dirichlet_instance(N,P,R,concentration_parameter,density_of_b1,b2maxfactor,b3max)

run_methods(α_prior,N,P,R,Xp,b1,b2,b3,α_tgt,precision,MP_time_limit,QA_max_time,QA_max_iterations,QA_termination_tolerance)
