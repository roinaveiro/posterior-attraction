...
def kl_normal_normal_given_X(X,μ_prior,Λ_prior,Λ_data_generating,Λ_post,μ_tgt,Λ_tgt,P): 
    ...
    δE=kl_bivariate_normal_normal(μ_post,Λ_post,μ_tgt,Λ_tgt,P)
    return δE
...
def plot_bivariate_normals_with_poisoningXpX(i_count,b1_name,b2max,b3max,μ_prior,Σ_prior,μ_post,Σ_post,μ_tgt,Σ_tgt,μ_poisoned,title,Xp,X):
    ...
    Xp = np.array(Xp)
    X = np.array(X)    
    if Xp.shape[0]>50:  # Don't graph dense plots
        return
    #if i_count!=2: # only plot iteration #3
    #    return
    ...
    # Mark the mean points for each distribution
    plt.scatter(μ_prior[0],μ_prior[1],color='blue',zorder=5)
    plt.text(μ_prior[0] + 0.2,μ_prior[1],'μ_prior',color='blue',fontsize=font_size_param)
    plt.scatter(μ_post[0],μ_post[1],color='green',zorder=5)
    plt.text(μ_post[0] + 1,μ_post[1],'μ_posterior',color='green',fontsize=font_size_param)
    plt.scatter(μ_tgt[0],μ_tgt[1],color='red',zorder=5)
    plt.text(μ_tgt[0]-2,μ_tgt[1],'μ_target',color='red',fontsize=font_size_param)
    plt.scatter(μ_poisoned[0],μ_poisoned[1],color='purple',zorder=5)
    ...
    for i in range(len(Xp)):
        ...
        plt.scatter(Xp[i,0],Xp[i,1],color='lightblue',edgecolor='black',s=100,label='Clean Data' if i == 0 else "",zorder=7)
        plt.scatter(X[i,0],X[i,1],color='red',edgecolor='black',s=100,label='Attacked Data' if i == 0 else "",zorder=6)
    ...
    plt.savefig(f"C:/Users/brian/Documents/PA/Plots/NN_EPA_N{N}_b1_{b1_name}_b2_{b2max}_b3_{b3max}_i{i_count+1}_{title}.png", dpi=300, bbox_inches='tight')     
    plt.show()

def solve_P1L_δE(N,P,μ_prior,Σ_prior,Λ_prior,Σ_data_generating,Λ_data_generating,Xp,Σ_post,Λ_post,μ_tgt,Σ_tgt,Λ_tgt,b1,b2,b3,precision,MP_time_limit,solver_name):
    ...
    # Formulate model
    # Objective function 
    model.objfnvalue = pyo.Objective(expr =0.5*(sum(sum((model.μ_tgt[i]-model.μ_post[i])*model.Λ_tgt[i,j]*(model.μ_tgt[j]-model.μ_post[j]) for i in model.P) for j in model.P)
                                     + sum(sum(model.Λ_tgt[i,j]*model.Σ_post[j,i] for i in model.P) for j in model.P)
                                     - np.log(np.linalg.det(Σ_post)/np.linalg.det(Σ_tgt))-P),sense=pyo.minimize)
    ...
    if (attempt=='success'):
        solution={
            ...
            'δE(X)':pyo.value(model.objfnvalue),           
...
def solve_Q1L1_for_Q1L(N,P,Xest,Xprime,μ_prior,Λ_prior,Λ_data_generating,Σ_post,Λ_post,μ_tgt,Λ_tgt,b1,b2,b3,MP_time_limit,precision,solver_name):
    start_time = time.time() # Start the clock
...
def solve_Q1L(N,P,Xprime,μ_prior,Λ_prior,Λ_data_generating,Σ_post,Λ_post,μ_tgt,Λ_tgt,b1,b2,b3,MP_time_limit,precision,QA_max_time,QA_max_iterations,QA_termination_tolerance,solver_name):
    stored_solutions={}
    k_iter=1
    ...
    while (k_iter<=QA_max_iterations and elapsed_time<QA_max_time and termination_check>QA_termination_tolerance):
        solution_Q1L1=solve_Q1L1_for_Q1L(N,P,Xest,Xprime,μ_prior,Λ_prior,Λ_data_generating,Σ_post,Λ_post,μ_tgt,Λ_tgt,b1,b2,b3,MP_time_limit,precision,solver_name)
        stored_solutions[k_iter]=solution_Q1L1
        if k_iter==1: 
            f_kminus1=kl_normal_normal_given_X(solution_Q1L1['X'],μ_prior,Λ_prior,Λ_data_generating,Λ_post,μ_tgt,Λ_tgt,P)+QA_termination_tolerance+1 
        else:
            f_kminus1=f_k
        f_k=kl_normal_normal_given_X(solution_Q1L1['X'],μ_prior,Λ_prior,Λ_data_generating,Λ_post,μ_tgt,Λ_tgt,P)
...
def solve_2M1(N,P,μ_prior,Σ_prior,Λ_prior,Σ_data_generating,Λ_data_generating,Xp,Σ_post,Λ_post,μ_tgt,Σ_tgt,Λ_tgt,b1,b2,b3,precision,MP_time_limit,solver_name):
    start_time = time.time() # Start the clock
    # Create a Pyomo model
    model = pyo.ConcreteModel()
    # Sets
...
def FGA1(N,P,Xp,b1,b2,b3,μ_prior,Λ_prior,Λ_data_generating,Λ_post,μ_tgt,Λ_tgt,precision): 
    start_time = time.time()  # Start the clock
    ...
    solution = {
        ...
        'δE(X)': float(kl_normal_normal_given_X(X,μ_prior,Λ_prior,Λ_data_generating,Λ_post,μ_tgt,Λ_tgt,P)),
        'time': round(elapsed_time,precision)
    }
...
def run_methods(i,b1_rho,b1_name,b2max,b3max,μ_prior,Σ_prior,Λ_prior,N,P,μ_data_generating,Σ_data_generating,Λ_data_generating,Xp,Xbar_data,b1,b2,b3,μ_tgt,Σ_tgt,Λ_tgt,MP_time_limit,QA_max_time,QA_max_iterations,QA_termination_tolerance,kl_function,SCD_epsilon,SCD_verbose,SCD_max_oscillations,SCD_second_order,PGD_lr,PGD_init_noise,PGD_max_iter,PGD_verbose,PGD_tolerance,PGD_no_change_steps):
    ...
    for solver_name in sorted(solvers):    
        #print('******************************')
        print(f"P1L Results with {solver_name}")
        solution_P1L=solve_P1L_δE(N,P,μ_prior,Σ_prior,Λ_prior,Σ_data_generating,Λ_data_generating,Xp,Σ_post,Λ_post,μ_tgt,Σ_tgt,Λ_tgt,b1,b2,b3,precision,MP_time_limit,solver_name)
        X=solution_P1L['X']
        Xbar=np.mean(X,axis=0)
        μ_poisoned=Σ_post @ (Λ_prior @ μ_prior+N*Λ_data_generating @ Xbar)
        δE_poisoned=round(kl_bivariate_normal_normal(μ_poisoned,Λ_post,μ_tgt,Λ_tgt,P),precision)
        #print(f"μ_poisoned={μ_poisoned}")
        print(f"-δE(X)={δE_poisoned}")
        #print(f"time={solution_P1L['time']} sec")
        title='P1L-'+solver_name[0]
        ...
        δE_poisoned_list[f"{title} KL"]=float(δE_poisoned)
        solve_time_list[f"{title} time"]=solution_P1L['time']
    ...
    for solver_name in sorted(solvers):    
        ...
        (solution_Q1L,Q1L_trace)=solve_Q1L(N,P,Xp,μ_prior,Λ_prior,Λ_data_generating,Σ_post,Λ_post,μ_tgt,Λ_tgt,b1,b2,b3,MP_time_limit,precision,QA_max_time,QA_max_iterations,QA_termination_tolerance,solver_name)
        X=solution_Q1L['X']
        Xbar=np.mean(X,axis=0)
        μ_poisoned=Σ_post @ (Λ_prior @ μ_prior+N*Λ_data_generating @ Xbar)
        δE_poisoned=round(kl_bivariate_normal_normal(μ_poisoned,Λ_post,μ_tgt,Λ_tgt,P),precision)
        ...
        plot_bivariate_normals_with_poisoningXpX(i,b1_name,b2max,b3max,μ_prior,Σ_prior,μ_post,Σ_post,μ_tgt,Σ_tgt,μ_poisoned,title,Xp,X)
        δE_poisoned_list[f"{title} KL"]=float(δE_poisoned)
        solve_time_list[f"{title} time"]=solution_Q1L['time']
        (num_changed_rows,num_changed,Eucl_dist)=compute_change_metrics(Xp,X)
        num_changed_rows_list[f"{title} rows chng"]=num_changed_rows
        num_changed_list[f"{title} entries chng"]=num_changed
        Eucl_dist_list[f"{title} L2"]=float(round(Eucl_dist,precision))
    ...
    for solver_name in sorted(solvers):    
        #print('******************************')
        print(f"Q1L1 Results with {solver_name}")
        solution_Q1L1=solve_Q1L1_for_Q1L(N,P,Xp,Xp,μ_prior,Λ_prior,Λ_data_generating,Σ_post,Λ_post,μ_tgt,Λ_tgt,b1,b2,b3,MP_time_limit,precision,solver_name)
        X=solution_Q1L1['X']
        Xbar=np.mean(X,axis=0)
        μ_poisoned=Σ_post @ (Λ_prior @ μ_prior+N*Λ_data_generating @ Xbar)
        δE_poisoned=round(kl_bivariate_normal_normal(μ_poisoned,Λ_post,μ_tgt,Λ_tgt,P),precision)
        #print(f"μ_poisoned={μ_poisoned}")
        print(f"-δE(X)={δE_poisoned}")
        #print(f"time={solution_Q1L1['time']} sec")
        title='Q1L1-'+solver_name[0]
        #plot_bivariate_normals_with_poisoning(μ_prior,Σ_prior,μ_post,Σ_post,μ_tgt,Σ_tgt,μ_poisoned,title)
        plot_bivariate_normals_with_poisoningXpX(i,b1_name,b2max,b3max,μ_prior,Σ_prior,μ_post,Σ_post,μ_tgt,Σ_tgt,μ_poisoned,title,Xp,X)
        δE_poisoned_list[f"{title} KL"]=float(δE_poisoned)
        solve_time_list[f"{title} time"]=solution_Q1L1['time']
        (num_changed_rows,num_changed,Eucl_dist)=compute_change_metrics(Xp,X)
        num_changed_rows_list[f"{title} rows chng"]=num_changed_rows
        num_changed_list[f"{title} entries chng"]=num_changed
        Eucl_dist_list[f"{title} L2"]=float(round(Eucl_dist,precision))
    ...
    for solver_name in sorted(solvers):    
        #print('******************************')
        print(f"2M1 Results with {solver_name}")
        solution_2M1=solve_2M1(N,P,μ_prior,Σ_prior,Λ_prior,Σ_data_generating,Λ_data_generating,Xp,Σ_post,Λ_post,μ_tgt,Σ_tgt,Λ_tgt,b1,b2,b3,precision,MP_time_limit,solver_name)
        X=solution_2M1['X']
        Xbar=np.mean(X,axis=0)
        μ_poisoned=Σ_post @ (Λ_prior @ μ_prior+N*Λ_data_generating @ Xbar)
        δE_poisoned=round(kl_bivariate_normal_normal(μ_poisoned,Λ_post,μ_tgt,Λ_tgt,P),precision)
        #print(f"μ_poisoned={μ_poisoned}")
        #print(f"2M1(X)={np.sqrt(solution_2M1['2M(X)'])}")
        print(f"-δE(X)={δE_poisoned}")
        #print(f"time={solution_2M1['time']} sec")
        title='2M1-'+solver_name[0]
        #plot_bivariate_normals_with_poisoning(μ_prior,Σ_prior,μ_post,Σ_post,μ_tgt,Σ_tgt,μ_poisoned,title)
        plot_bivariate_normals_with_poisoningXpX(i,b1_name,b2max,b3max,μ_prior,Σ_prior,μ_post,Σ_post,μ_tgt,Σ_tgt,μ_poisoned,title,Xp,X)
        δE_poisoned_list[f"{title} KL"]=float(δE_poisoned)
        solve_time_list[f"{title} time"]=solution_2M1['time']
        (num_changed_rows,num_changed,Eucl_dist)=compute_change_metrics(Xp,X)
        num_changed_rows_list[f"{title} rows chng"]=num_changed_rows
        num_changed_list[f"{title} entries chng"]=num_changed
        Eucl_dist_list[f"{title} L2"]=float(round(Eucl_dist,precision))
    ...
    #print('******************************')
    print('FGA1 Results')
    solution_FGA1=FGA1(N,P,Xp,b1,b2,b3,μ_prior,Λ_prior,Λ_data_generating,Λ_post,μ_tgt,Λ_tgt,precision)
    ...
    #print(f"μ_poisoned={μ_poisoned}")
    print(f"-δE(X)={δE_poisoned}")
    #print('time=',solution_FGA1['time'],'sec')
    title='FGA1'
    #plot_bivariate_normals_with_poisoning(μ_prior,Σ_prior,μ_post,Σ_post,μ_tgt,Σ_tgt,μ_poisoned,title)
    plot_bivariate_normals_with_poisoningXpX(i,b1_name,b2max,b3max,μ_prior,Σ_prior,μ_post,Σ_post,μ_tgt,Σ_tgt,μ_poisoned,title,Xp,X)
    δE_poisoned_list[f"{title} KL"]=float(δE_poisoned)
    solve_time_list[f"{title} time"]=solution_FGA1['time']
    (num_changed_rows,num_changed,Eucl_dist)=compute_change_metrics(Xp,X)
    num_changed_rows_list[f"{title} rows chng"]=num_changed_rows
    num_changed_list[f"{title} entries chng"]=num_changed
    Eucl_dist_list[f"{title} L2"]=float(round(Eucl_dist,precision))
    ...
    results={
        ...
        'tgt mean':μ_tgt,
        'tgt cov':Σ_tgt,
        'KL_poisoned_list':δE_poisoned_list,
...
QA_max_time=60 # max time for Q1L process
...
b1_rho_values=[0.67]#0.33,0.67,1]
b2max_values=[3]#1,2,3]
b3max_values=[5]#1,3,5]
...
for b1_rho_counter,b1_rho in enumerate(b1_rho_values, 1):
    ...
    for b2max_counter,b2max in enumerate(b2max_values,1):  
        for b3max_counter,b3max in enumerate(b3max_values,1):        
            ...
            file_path_aggregate=f"C:/Users/brian/Documents/PA/NN_EPA_N{N}_b1_{b1_name}_b2_{b2max}_b3_{b3max}_k{k}.csv"
