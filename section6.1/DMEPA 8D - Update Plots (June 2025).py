...
def plot_heatmap(x, title, i):
    """
    Plots a heatmap for a 10x8 NumPy array, standardized with a color scale from -3 to 3.
    ...
    # Round values and convert to int for integer display
    ...
    ax.set_title(f"Changes to X' by {title}", fontsize=title_fontsize)
    ax.set_xlabel("Data Point (p)", fontsize=label_fontsize)
    ax.set_ylabel("Observation (n)", fontsize=label_fontsize)
    ...
    plt.tight_layout()
    plt.savefig(f"C:/Users/brian/Documents/PA/Heatmaps/DM_EPA_iter{i+1}_delta_X_{title}.png", dpi=300, bbox_inches='tight')
    plt.show()
...
def plot_dirichlet_distributions(α_prior, α_post, α_tgt,i):
    ...
    for alpha, label in zip(alphas, labels):
        ...
        contour = ax.tricontourf(U, V, density, levels=100, cmap=cmap,vmin=0,vmax=45)
        #ax.set_title(label)
        ...
        plt.savefig(f"C:/Users/brian/Documents/PA/Plots/DM_EPA_iter{i+1}_{label[9:14]}.png", dpi=300, bbox_inches='tight')     
        plt.show()
...
def plot_poisoned_dirichlet_distributions(α_poisoned, title,i):
    ...
    for alpha, label in zip(alphas, labels):
        ...
        cbar.ax.yaxis.set_major_formatter(plt.FuncFormatter(lambda x, _: f"{int(x):d}"))
        cbar.set_label("Density", fontsize=16)
        plt.savefig(f"C:/Users/brian/Documents/PA/Plots/DM_EPA_iter{i+1}_{title}.png", dpi=300, bbox_inches='tight')     
        plt.show()
...
def run_methods(α_prior,N,P,i,R,Xp,b1,b2,b3,α_tgt,precision,MP_time_limit,QA_max_time,QA_max_iterations,QA_termination_tolerance):
    ...
    δE_poisoned_list = results.pop("KL_poisoned")
...
file_path_aggregate=f"C:/Users/brian/Documents/PA/DM_EPA_N{N}_P{P}_k{k}.csv"
