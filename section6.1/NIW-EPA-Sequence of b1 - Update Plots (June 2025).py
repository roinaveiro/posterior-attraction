...
def kl_normal_inverse_wishart_given_X(μ_prior,κ_prior,v_prior,Λ_prior,X,μ_tgt,κ_tgt,v_tgt,Λ_tgt):
    ...
    δE=kl_normal_inverse_wishart(μ_pert,κ_pert,v_pert,Λ_pert,μ_tgt,κ_tgt,v_tgt,Λ_tgt)
    return δE
...
def plot_bivariate_normals(μ_prior,Σ_prior,μ_post,Σ_post,μ_tgt,Σ_tgt,N):
    ...
    plt.text(μ_prior[0] + 0.2,μ_prior[1]+0.5,'μ_prior',color='blue',fontsize=font_size_param)
    plt.scatter(μ_post[0],μ_post[1],color='green',zorder=5)
    plt.text(μ_post[0] + 1,μ_post[1]+0.5,'μ_post',color='green',fontsize=font_size_param)
    plt.scatter(μ_tgt[0],μ_tgt[1],color='red',zorder=5)
    plt.text(μ_tgt[0] + 1,μ_tgt[1],'μ_tgt',color='red',fontsize=font_size_param)
    # Add labels and title
    plt.title('Prior, Posterior, and Target Distributions')
    plt.xlabel(r'$x_1$',fontsize=font_size_param)
    plt.ylabel(r'$x_2$',fontsize=font_size_param)
    plt.grid(True)
    plt.tick_params(axis='both', which='major', labelsize=font_size_param)
    plt.savefig(f"C:/Users/brian/Documents/PA/Plots/NIW_EPA_N{N}_Unpoisoned.png", dpi=300, bbox_inches='tight')     
    plt.show()
    return
...
def plot_bivariate_normals_with_poisoning(μ_prior,Σ_prior,μ_post,Σ_post,μ_tgt,Σ_tgt,μ_poisoned,Σ_poisoned,title,N,b1):
    ...
    plt.text(μ_poisoned[0] + 1,μ_poisoned[1],'μ_poisoned',color='purple',fontsize=font_size_param)
    # Add labels and title
...
def plot_bivariate_normals_with_poisoningXpX(μ_prior,Σ_prior,μ_post,Σ_post,μ_tgt,Σ_tgt,μ_poisoned,Σ_poisoned,title,Xp,X,N,b1):
    ...
    plt.text(μ_post[0] + 1,μ_post[1]+0.5,'μ_post',color='green',fontsize=font_size_param)
    plt.scatter(μ_tgt[0],μ_tgt[1],color='red',zorder=5)
    plt.text(μ_tgt[0] + 1,μ_tgt[1]-0.5,'μ_tgt',color='red',fontsize=font_size_param)
    plt.scatter(μ_poisoned[0],μ_poisoned[1],color='purple',zorder=5)
    plt.text(μ_poisoned[0] + 1,μ_poisoned[1],'μ_poisoned',color='purple',fontsize=font_size_param)
    ...
    for i in range(len(Xp)):
        ...
        plt.scatter(Xp[i,0],Xp[i,1],color='lightblue',edgecolor='black',s=100,label='Clean Data' if i == 0 else "",zorder=6)
        plt.scatter(X[i,0],X[i,1],color='red',edgecolor='black',s=100,label='Attacked Data' if i == 0 else "",zorder=7)
        ...
        if np.any(np.abs(Xp[i] - X[i]) > 0.01):  
            ...
            plt.arrow(
                ...
                color='grey',head_width=0.1,linewidth=2.5,length_includes_head=True,zorder=5
            )
    ...
    plt.grid(True)
    plt.tick_params(axis='both', which='major', labelsize=font_size_param)
    plt.legend(fontsize=font_size_param)
    plt.savefig(f"C:/Users/brian/Documents/PA/Plots/NIW_EPA_N{N}_Poisoned_b1_{b1}.png", dpi=300, bbox_inches='tight')     
    plt.show()
        ...
        for p in range(P):
            for k in range(N):
                for l in range(P):
                    ...
                    μ_np_minus, κ_np_minus, v_np_minus, Λ_np_minus = posterior_normal_inverse_wishart(μ_prior, κ_prior, v_prior, Λ_prior, X_np_minus)
                    kl_np_minus = kl_normal_inverse_wishart(μ_np_minus, κ_np_minus, v_np_minus, Λ_np_minus, μ_tgt, κ_tgt, v_tgt, Λ_tgt)
                    ...
                    μ_kl_minus, κ_kl_minus, v_kl_minus, Λ_kl_minus = posterior_normal_inverse_wishart(μ_prior, κ_prior, v_prior, Λ_prior, X_kl_minus)
                    kl_kl_minus = kl_normal_inverse_wishart(μ_kl_minus, κ_kl_minus, v_kl_minus, Λ_kl_minus, μ_tgt, κ_tgt, v_tgt, Λ_tgt)
                    ...
                    μ_np_kl_minus, κ_np_kl_minus, v_np_kl_minus, Λ_np_kl_minus = posterior_normal_inverse_wishart(μ_prior, κ_prior, v_prior, Λ_prior, X_np_kl_minus)
                    kl_np_kl_minus = kl_normal_inverse_wishart(μ_np_kl_minus, κ_np_kl_minus, v_np_kl_minus, Λ_np_kl_minus, μ_tgt, κ_tgt, v_tgt, Λ_tgt)
...
def solve_Q1L1_for_Q1L(zN,P,μ_prior,κ_prior,v_prior,Λ_prior,μ_data_generating,Σ_data_generating,Λ_data_generating,Xprime,Xest,Xbar_data,b1,b2,b3,μ_tgt,κ_tgt,v_tgt,Λ_tgt,MP_time_limit,solver_name):
    ...
    try:
        if (solver_name=='CPLEX'):
            ...
            solver.options['optimalitytarget'] = 3  # Solves a non-convex Q1L1
...
def solve_Q1L(N,P,μ_prior,κ_prior,v_prior,Λ_prior,μ_data_generating,Σ_data_generating,Λ_data_generating,Xp,Xbar_data,b1,b2,b3,μ_tgt,κ_tgt,v_tgt,Λ_tgt,MP_time_limit,solver_name):
    stored_solutions={}
    k_iter=1
    ...
    while (k_iter<=SA_max_iterations and elapsed_time<SA_max_time and termination_check>SA_termination_tolerance):
        solution_Q1L1=solve_Q1L1_for_Q1L(N,P,μ_prior,κ_prior,v_prior,Λ_prior,μ_data_generating,Σ_data_generating,Λ_data_generating,Xp,Xest,Xbar_data,b1,b2,b3,μ_tgt,κ_tgt,v_tgt,Λ_tgt,MP_time_limit,solver_name)
        stored_solutions[k_iter]=solution_Q1L1
        if k_iter==1:
            f_kminus1=kl_normal_inverse_wishart_given_X(μ_prior,κ_prior,v_prior,Λ_prior,np.array(solution_Q1L1['X']),μ_tgt,κ_tgt,v_tgt,Λ_tgt)+SA_termination_tolerance+1 
        else:
            f_kminus1=f_k
        f_k=kl_normal_inverse_wishart_given_X(μ_prior,κ_prior,v_prior,Λ_prior,np.array(solution_Q1L1['X']),μ_tgt,κ_tgt,v_tgt,Λ_tgt)
        ...
        if f_k<f_kminus1: # only accept a better solution
            ...
            Xest=solution_Q1L1['X']
            solution=solution_Q1L1
        else:
            print(f"-Rejecting a worse solution on iteration {k_iter}")
        k_iter=k_iter+1
        end_time = time.time() # Stop the clock
        elapsed_time = end_time - start_time # Calculate the elapsed time
    # make sure the reported time is the cumulative time,not the time of the last Q1L1 solution
...
def generate_target_distribution(μ_prior,κ_prior,v_prior,Λ_prior,N,P,μ_data_generating,Σ_data_generating,Λ_data_generating,Xp,Xbar_data,b1,b2,b3,μ_tgt,κ_tgt,v_tgt,Λ_tgt,MP_time_limit,SA_max_time,SA_max_iterations,SA_termination_tolerance):
    ...
    for solver_name in solvers:
        print('******************************')
        print(f"Q1L Results with {solver_name} to generate target distribution")
        (solution_Q1L,Q1L_trace)=solve_Q1L(N,P,μ_prior,κ_prior,v_prior,Λ_prior,μ_data_generating,Σ_data_generating,Λ_data_generating,Xp,Xbar_data,b1,b2,b3,μ_tgt,κ_tgt,v_tgt,Λ_tgt,MP_time_limit,solver_name)
        X=solution_Q1L['X']
        (μ_poisoned,κ_poisoned,v_poisoned,Λ_poisoned)=posterior_normal_inverse_wishart(μ_prior,κ_prior,v_prior,Λ_prior,X)
        δE_poisoned=round(kl_normal_inverse_wishart(μ_poisoned,κ_poisoned,v_poisoned,Λ_poisoned,μ_tgt,κ_tgt,v_tgt,Λ_tgt),precision)
        #print(f"μ_poisoned={μ_poisoned}")
        print(f"δE(X)={δE_poisoned}")
        #print(f"time={solution_Q1L['time']} sec")
        title='Q1L-'+solver_name[0]
...
def run_methods(μ_prior,κ_prior,v_prior,Λ_prior,N,P,μ_data_generating,Σ_data_generating,Λ_data_generating,Xp,Xbar_data,b1,b2,b3,μ_tgt,κ_tgt,v_tgt,Λ_tgt,MP_time_limit,SA_max_time,SA_max_iterations,SA_termination_tolerance):
   ...
   for solver_name in solvers:
       #print('******************************')
       print(f"Q1L Results with {solver_name}")
       (solution_Q1L,Q1L_trace)=solve_Q1L(N,P,μ_prior,κ_prior,v_prior,Λ_prior,μ_data_generating,Σ_data_generating,Λ_data_generating,Xp,Xbar_data,b1,b2,b3,μ_tgt,κ_tgt,v_tgt,Λ_tgt,MP_time_limit,solver_name)
       X=solution_Q1L['X']
       (μ_poisoned,κ_poisoned,v_poisoned,Λ_poisoned)=posterior_normal_inverse_wishart(μ_prior,κ_prior,v_prior,Λ_prior,X)
       δE_poisoned=float(round(kl_normal_inverse_wishart(μ_poisoned,κ_poisoned,v_poisoned,Λ_poisoned,μ_tgt,κ_tgt,v_tgt,Λ_tgt),precision))
       #print(f"μ_poisoned={μ_poisoned}")
       print(f"δE(X)={δE_poisoned}")
       print(f"time={solution_Q1L['time']} sec")
       title='Q1L-'+solver_name[0]+' b1='+str(b1/N)
       #plot_bivariate_normals_with_poisoning(μ_prior,Λ_prior/(κ_prior*(v_prior-3)),μ_post,Λ_post/(κ_post*(v_post-3)),μ_tgt,Λ_tgt/(κ_tgt*(v_tgt-3)),μ_poisoned,Λ_poisoned/(κ_poisoned*(v_poisoned-3)),title,N)
       plot_bivariate_normals_with_poisoningXpX(μ_prior,Λ_prior/(κ_prior*(v_prior-3)),μ_post,Λ_post/(κ_post*(v_post-3)),μ_tgt,Λ_tgt/(κ_tgt*(v_tgt-3)),μ_poisoned,Λ_poisoned/(κ_poisoned*(v_poisoned-3)),title,Xp,X,N,b1)

   l2_norm = np.linalg.norm(μ_poisoned - μ_tgt)
   frobenius_norm = np.linalg.norm(Λ_post - Λ_tgt, 'fro')
   return (δE_poisoned,solution_Q1L['time'],l2_norm,frobenius_norm)
...
def plot_dictionary(data,y_axis_title,N,file_title):
    ...
    plt.xlabel(r'$\rho$',fontsize=font_size_param)
    plt.ylabel(y_axis_title,fontsize=font_size_param)
    plt.xlim(0, 1)  # Set x-axis range
    plt.ylim(bottom=0)    # Set y-axis lower bound
    plt.grid(True)
    plt.savefig(f"C:/Users/brian/Documents/PA/Plots/NIW_EPA_N{N}_{file_title}.png", dpi=300, bbox_inches='tight')     
    plt.tick_params(axis='both', which='major', labelsize=font_size_param)
    plt.show()
    return
...
def dump_dictionaries_to_file(dict1,dict2,dict3,dict4):
    ...
    # Write to a CSV file
    with open('/Users/brian/Documents/PA/Plots/NIW_EPA_output.csv', mode='w', newline='') as file:
...
b3max=3000 # maximum magnitude of any change in an entry k; for now,we've assumed it's 5,for simplicity
...
#b1_rho_values=[0.05,0.35,0.55,1.0]
...
for b1_rho_counter,b1_rho in enumerate(b1_rho_values, 1): 
    ...
    (Xp,Xbar_data,b1,b2,b3)=generate_bivariate_instance(N,P,μ_data_generating,Σ_data_generating,b1_rho,b2max,b3max,precision)
    (b1_δE,b1_time,b1_l2_norm,b1_frobenius_norm)=run_methods(μ_prior,κ_prior,v_prior,Λ_prior,N,P,μ_data_generating,Σ_data_generating,Λ_data_generating,Xp,Xbar_data,b1,b2,b3,μ_tgt,κ_tgt,v_tgt,Λ_tgt,MP_time_limit,SA_max_time,SA_max_iterations,SA_termination_tolerance)
    stored_b1_δE[b1_rho]=b1_δE
...
plot_dictionary(stored_b1_δE,r'$\delta_{E}$',N,'EPA_KL_dist')
plot_dictionary(stored_b1_time,'Required Comp. Effort (sec)',N,'EPA_times')
plot_dictionary(stored_b1_L2_norm,r'L2 norm',N,'EPA_L2_norms')
plot_dictionary(stored_b1_Frobenius,r'Frobenius norm',N,'EPA_Frob_norms')
dump_dictionaries_to_file(stored_b1_δE,stored_b1_time,stored_b1_L2_norm,stored_b1_Frobenius)
print(f'μ_tgt={μ_tgt}')
print(f'κ_tgt={κ_tgt}')
print(f'v_tgt={v_tgt}')
print(f'Λ_tgt={Λ_tgt}')

#μ_tgt=[0.52706183 2.01191807]
#κ_tgt=11
#v_tgt=15
#Λ_tgt=[[11.48816554  5.33724454]
# [ 5.33724454 10.6360291 ]]