...
def kl_dirichlet_multinomial_given_X(α,X,α_given):
    return dirichlet_kl_divergence(α+np.sum(X,axis=0),α_given)
...
def plot_dirichlet_distributions(α_prior, α_post, α_tgt,i):
    ...
    labels = [
        f"Prior {format_alpha(α_prior)}",
        f"Posterior {format_alpha(α_post)}",
        f"Target {format_alpha(α_tgt)}",
    ]
    ...
    for alpha, label in zip(alphas, labels):
        ...
        plt.savefig(f"C:/Users/brian/Documents/PA/Plots/DM_EPA_iter{i}_{label[0:5]}.png", dpi=300, bbox_inches='tight')     
        plt.show()
...
def plot_poisoned_dirichlet_distributions(α_poisoned, title,i):
    ...
    alphas = [α_poisoned]
    labels = [
        f"{title} Poisoned {format_alpha(α_poisoned)}"
    ]
    cmap = plt.cm.coolwarm
    ...
    for alpha, label in zip(alphas, labels):
        ...
        cbar = fig.colorbar(contour, ax=ax, shrink=0.8)
        cbar.set_label("Density")
        plt.savefig(f"C:/Users/brian/Documents/PA/Plots/DM_EPA_iter{i}_{title}.png", dpi=300, bbox_inches='tight')     
        plt.show()
...
def approximate_gradient(α_prior, Xp, α_tgt, precision):
    ...
    epsilon = 1#e-5  # Small perturbation for numerical stability
...
def solve_Q1L1_for_Q1L(N,P,Xest,α_prior,Xprime,b1,b2,b3,α_tgt,precision,MP_time_limit,solver_name):
...
def solve_Q1L(N,P,α_prior,Xprime,b1,b2,b3,α_tgt,precision,MP_time_limit,solver_name,QA_max_time,QA_max_iterations,QA_termination_tolerance):
    stored_solutions={}
    k_iter=1
    ...
    while (k_iter<=QA_max_iterations and elapsed_time<QA_max_time and termination_check>QA_termination_tolerance):
        solution_Q1L1=solve_Q1L1_for_Q1L(N,P,Xest,α_prior,Xprime,b1,b2,b3,α_tgt,precision,MP_time_limit,solver_name)
        stored_solutions[k_iter]=solution_Q1L1
        if k_iter==1: 
            f_kminus1=kl_dirichlet_multinomial_given_X(α_prior,solution_Q1L1['X'],α_tgt)+QA_termination_tolerance+1 
        else:
            f_kminus1=f_k
        f_k=kl_dirichlet_multinomial_given_X(α_prior,solution_Q1L1['X'],α_tgt)
...
def solve_2M1(N,P,α_prior,Xprime,b1,b2,b3,α_tgt,precision,MP_time_limit,solver_name):
    start_time = time.time() # Start the clock
    # Create a Pyomo model
    model = pyo.ConcreteModel()
    # Sets
...
def FGA1(N,P,α_prior,Xp,b1,b2,b3,α_tgt,precision):
    start_time = time.time()  # Start the clock
...
def run_methods(α_prior,N,P,i,R,Xp,b1,b2,b3,α_tgt,precision,MP_time_limit,QA_max_time,QA_max_iterations,QA_termination_tolerance):
    ...
    δ_kl_ex = round(dirichlet_kl_divergence(α_post,α_tgt),precision)
    print(f"α_tgt={α_tgt}")
    print(f"δE(X')={δ_kl_ex}")    
    if P==3:
        plot_dirichlet_distributions(α_prior,α_post,α_tgt,i)
    
    δE_poisoned_list={}
    α_poisoned_list={}
    solve_time_list={}
    δE_poisoned_list["Unpoisoned KL"]=δ_kl_ex
    
    for solver_name in solvers: 
        print('******************************')
        print(f"Q1L Results with {solver_name}")
        (solution_Q1L,Q1L_trace)=solve_Q1L(N,P,α_prior,Xp,b1,b2,b3,α_tgt,precision,MP_time_limit,solver_name,QA_max_time,QA_max_iterations,QA_termination_tolerance)
        X=np.array(solution_Q1L['X'])
        #print(f"X={X}")
        α_poisoned=α_prior+np.sum(X,axis=0)
        print(f"α_poisoned={α_poisoned}")
        δE_poisoned=round(dirichlet_kl_divergence(α_poisoned,α_tgt),precision)
        print(f"δE(X)={δE_poisoned}")
        print(f"time={solution_Q1L['time']} sec")
        title='Q1L-'+solver_name[0]
        if P==3:
            plot_poisoned_dirichlet_distributions(α_poisoned, title,i)
    ...
    for solver_name in solvers: 
        ...
        solution_2M1=solve_2M1(N,P,α_prior,Xp,b1,b2,b3,α_tgt,precision,MP_time_limit,solver_name)
        X=np.array(solution_2M1['X'])
        #print(f"X={X}")
        α_poisoned=α_prior+np.sum(X,axis=0)
        print(f"α_poisoned={α_poisoned}")
        δE_poisoned=round(dirichlet_kl_divergence(α_poisoned,α_tgt),precision)
    ...
    print('******************************')
    print('FGA1 Results')
    solution_FGA1=FGA1(N,P,α_prior,Xp,b1,b2,b3,α_tgt,precision)
    X=np.array(solution_FGA1['X'])
    #print(f"X={X}")
    α_poisoned=α_prior+np.sum(X,axis=0)
    print(f"α_poisoned={α_poisoned}")
    δE_poisoned=round(dirichlet_kl_divergence(α_poisoned,α_tgt),precision)
    print(f"δE(X)={δE_poisoned}")
    print(f"time={solution_FGA1['time']} sec")
    title='FGA1'
    if P==3:    
        plot_poisoned_dirichlet_distributions(α_poisoned, title,i)
    δE_poisoned_list[f"{title} KL"]=float(δE_poisoned)
...
P=3 # Number of categories for the Dirichlet distribution
R=6 # Number of replications within a single multinomial observations (R choose P with pmf p)
α_prior = np.array([1,2,8])
...
α_tgt = np.array([16,8,4])
density_of_b1=0.75 # approximate percentage of elements that can change
b2maxfactor=3 # when generating b2-values,imposes allows up to round(b2maxfactor*b1/K,0) or 1 change,whichever is larger
b3max=20 # maximum magnitude of any change in an entry k; for now,we've assumed it's 5,for simplicity
...
for i in range(k):
    ...
    (Xp,b1,b2,b3)=generate_dirichlet_instance(N,P,i,R,concentration_parameter,density_of_b1,b2maxfactor,b3max)
...
file_path_aggregate=f"C:/Users/brian/Documents/PA/DM_EPA_N{N}_k{k}.csv"
