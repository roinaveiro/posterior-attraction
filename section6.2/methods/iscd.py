...
class ISCD(BaseMethod):
    ...
    def __init__(
        ...
        Args:
            ...
            ensure_positive: If True, ensures that the resulting data is greater or equal than 0.
        """
        ...
        self.dummy_cols = dummy_cols if dummy_cols is not None else []
        ...
        while True:
            ...
            for key, tensor in data_dict.items():
                ...
                for idx in itertools.product(*(range(s) for s in tensor.shape)):
                    ...
                    if key == "X":
                        eps_here = self.constraints["X"]["epsilon_map"][idx].item()
                    else:
                        eps_here = self.epsilon          # rest still use global ε

                    if self.second_order and hdiag is not None:
                        h_val = hdiag[idx].item()
                        # second-order local improvement estimate
                        local_score = -abs(grad_val)*eps_here + 0.5*h_val*(eps_here**2)
                    else:
                        # first-order
                        local_score = -abs(grad_val)*eps_here
            ...
            # -- coordinate‑specific epsilon ---------------------------------
            if best_key == "X":
                eps_here = self.constraints["X"]["epsilon_map"][best_idx].item()
            else:
                eps_here = self.epsilon          # rest still use global ε

            perturbation = -eps_here * torch.sign(torch.tensor(best_grad_val))
            ...
            # (g) Apply update
            old_val = data_dict[best_key].data[best_idx].item()
            proposed_new_val = old_val + actual_perturb

            if best_key == "X" and best_idx[1] in self.dummy_cols:
                proposed_new_val = 1.0 if proposed_new_val >= 0.5 else 0.0

            if self.ensure_positive:
                proposed_new_val = max(proposed_new_val, 0.0)

            # Now compute the *real* net change
            real_perturb = proposed_new_val - old_val

            data_dict[best_key].data[best_idx] = proposed_new_val
            self.total_perturbation[best_key][best_idx] += real_perturb

            if real_perturb == 0:
                # clamp has nullified the move, so mask out and skip
                self._mask_coordinate(best_key, best_idx)
                if self.verbose:
                    print(f"No net movement for {best_key}[{best_idx}], masking out.")
                continue

            # If first time changing this coordinate, update "modified" + key-level count + row-level count
            if not self.modified[best_key][best_idx] and abs(real_perturb) > 0:
